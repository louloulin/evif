...
- **输出重定向**（`>`）：✅ **已实现**。将命令输出写入本地文件（覆盖模式）。
  ```bash
  # REPL 内
  cat /mem/file.txt > local_copy.txt
  ls /mem > file_list.txt
  echo hello > greeting.txt
  ```
- **追加重定向**（`>>`）：✅ **已实现**。将命令输出追加到本地文件末尾。
  ```bash
  cat /mem/log1.txt >> all_logs.txt
  echo "new entry" >> log.txt
  ```
- **输入重定向**（`<`）：✅ **已实现**。从本地文件读取内容，作为 `write` 命令的输入。
  ```bash
  # 将本地文件内容写入 EVIF 路径
  write /mem/uploaded.txt < local_file.txt
  ```

### 2.3 变量与控制流

- **变量替换**（如 `$path`、`${var}`）：✅ **已实现**（REPL 和脚本内）。
- **控制流**（如 `if`/`else`、`for`、`while`）：✅ **已实现**（脚本内）。
...
| 重定向       | 支持           | ✅ 已实现（>, >>, <） |
| 变量/控制流  | 支持           | ✅ 已实现（REPL/脚本） |
| 脚本         | 支持           | 部分/简化            |

EVIF CLI 定位为**单命令 + 可选 REPL**，满足对 EVIF 服务的常用文件/挂载操作。已支持重定向（`>`、`>>`、`<`）、变量替换和控制流。
...
**文档版本**：与 EVIF 2.4 Phase 10.2 对应；CLI 支持重定向（`>`、`>>`、`<`）、变量替换和控制流。
