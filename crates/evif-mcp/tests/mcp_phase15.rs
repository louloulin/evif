...
async fn mcp15_claude_md_default_params() {
    ...
    assert!(schema.get("properties").unwrap().get("project_path").is_some());
    assert!(schema.get("properties").unwrap().get("include_skills").is_some());
    assert!(schema.get("properties").unwrap().get("include_context").is_some());
}
...
async fn mcp15_claude_md_returns_content() {
    ...
    assert!(content.contains("Auto-generated by EVIF MCP Server"), "Should be auto-generated");
}
...
async fn mcp15_claude_md_with_skills_disabled() {
    ...
    assert!(!content.contains("## Skills"), "Skills section should be omitted");
}
