...
enum FaissCommand {
    Add {
        ...
        response: mpsc::Sender<MemResult<()>>,
    },
    AddBatch {
    ...
    Search {
        ...
        response: mpsc::Sender<MemResult<Vec<SearchResult>>>,
    },
    Get {
        id: String,
        response: mpsc::Sender<MemResult<Option<Vec<f32>>>>,
    },
    Remove {
        id: String,
        response: mpsc::Sender<MemResult<()>>,
    ...
    Stats {
        response: mpsc::Sender<IndexStats>,
    },
}
                    ...
                    match cmd {
                        FaissCommand::Add {
                            ...
                            if vector.len() != dimension as usize {
                                response
                                    .send(Err(MemError::Vector("dimension mismatch".to_string())))
                                    .await
                                    .ok();
                                continue;
                            }
...
impl VectorIndex for FaissVectorIndex {
    async fn add(
        ...
        self.sender
            ...
            .await
            .map_err(|e| MemError::Vector(e.to_string()))?;
    ...
    async fn len(&self) -> usize {
        let (tx, mut rx) = mpsc::channel(1);
        self.sender
            .send(FaissCommand::Len { response: tx })
            .await
            .ok();
    ...
    async fn clear(&self) -> MemResult<()> {
        let (tx, mut rx) = mpsc::channel(1);
        self.sender
            .send(FaissCommand::Clear { response: tx })
            .await
            .map_err(|e| MemError::Vector(e.to_string()))?;
        rx.recv().await.ok().flatten().unwrap_or(Ok(()))
    }
    ...
    async fn stats(&self) -> IndexStats {
        let (tx, mut rx) = mpsc::channel(1);
        self.sender
            .send(FaissCommand::Stats { response: tx })
            .await
            .ok();
        rx.recv().await.unwrap_or(IndexStats {
            total_vectors: 0,
...
mod tests {
    ...
    async fn test_faiss_add_and_search() {
        ...
        index
            ...
            .unwrap();
        index
            .add("item3".to_string(), vec![0.0, 1.0, 0.0], None)
            .await
            .unwrap();
