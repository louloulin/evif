// 华为云对象存储服务 (OBS) Plugin
//
// 基于 OpenDAL 的华为云 OBS 插件
...
/// 华为云 OBS 插件
pub struct HuaweiObsFsPlugin {
    inner: OpendalPlugin,
}

impl HuaweiObsFsPlugin {
    /// 从配置创建华为云 OBS 插件
    pub async fn from_config(config: HuaweiObsConfig) -> EvifResult<Self> {
        let opendal_config = OpendalConfig {
            service: OpendalService::Obs,
            root: config.root,
            endpoint: config.endpoint,
            access_key: Some(config.access_key_id),
            secret_key: Some(config.secret_access_key),
...
impl EvifPlugin for HuaweiObsFsPlugin {
    fn name(&self) -> &str {
        "huaweiobs-fs"
    }
...
mod tests {
    ...
    #[tokio::test]
    #[cfg(feature = "opendal")]
    async fn test_huaweiobs_config() {
        let config = HuaweiObsConfig {
            ...
            endpoint: Some("obs.cn-north-4.myhuaweicloud.com".to_string()),
            ..Default::default()
        };

        assert_eq!(config.bucket, "test-bucket");
        assert_eq!(config.access_key_id, "test-access-key-id");
        assert_eq!(config.endpoint.as_deref(), Some("obs.cn-north-4.myhuaweicloud.com"));
    }
}
