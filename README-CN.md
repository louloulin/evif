...
> 基于 Rust 构建的强大、可扩展的图虚拟文件系统，遵循 Plan 9 "万物皆文件" 哲学。

[English](README.md)

## 概述

EVIF 是一个模块化的虚拟文件系统，通过插件架构为各种存储后端提供统一接口。它将图数据结构的强大功能与传统文件系统语义相结合，在保持 POSIX 兼容性的同时提供高级查询能力。

### 核心特性

- **插件架构**: 30+ 内置插件，支持各种存储后端
- **图引擎**: 先进的图数据结构，支持索引和查询
...
| **evif-graph** | 图数据结构、节点、边、索引、查询执行 |
| **evif-storage** | 可插拔存储后端（内存、Sled、RocksDB、S3） |
| **evif-vfs** | 虚拟文件系统抽象层 |
...
│   ├── evif-client/      # 客户端 SDK
│   ├── evif-fuse/        # FUSE 集成
│   ├── evif-grpc/        # gRPC 服务（已禁用）
│   ├── evif-auth/        # 认证层
│   ├── evif-macros/      # 过程宏
│   ├── evif-metrics/     # 指标收集
│   └── evif-plugins/     # 插件实现（30+）
├── docs/                  # 文档
├── benches/               # 基准测试
├── tests/                 # 集成测试
├── examples/              # 示例代码
